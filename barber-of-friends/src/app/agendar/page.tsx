...
export default function AgendarPage() {
  ...
  const unavailable = useMemo(
    () => booked.filter((appointment) => appointment.appointmentDate === day),
    [booked, day]
  );
  ...
  async function confirm() {
    ...
    try {
      ...
      setConfirmation({
        name: data.clientName || clientName || guestName.trim(),
    ...
    <main className="site-shell">
      <section className="page booking-page">
        ...
        <p className="page-subtitle">
          ...
          <strong>Domingo não possui agendamento.</strong>
        </p>
        ...
        <div className="calendar-row">
          {days.map((date) => (
            <button
              ...
              className={`day-card ${day === key(date) ? "active" : ""}`}
              onClick={() => {
                setDay(key(date));
                setTime("");
              }}
        ...
        <section className="booking-panel glass">
          ...
          <div className="service-grid">
            {services.map((item) => {
              const hasDiscount =
                item.name === "Corte premium" && loyaltyOffer;

              return (
                <button
                  key={item.name}
                  className={`service-card ${service === item.name ? "selected" : ""}`}
                ...
                >
                  ...
                  {hasDiscount ? (
                    <>
                      <small className="loyalty-discount-badge">
                        50% DE DESCONTO
                      </small>
                      <span>R$ 40,00</span>
                      <del>R$ 80,00</del>
          ...
          <p className="booking-hint">
            A cada 5 cortes concluídos com o barbeiro, o próximo corte premium
            recebe 50% de desconto.
          </p>
          ...
          {!logged && (
            <div className="guest-fields">
              ...
              <p>
                Não precisa de cadastro. Para reservar, informe somente seu
                nome.
              </p>
              ...
              <p>
                Quer acompanhar seu histórico depois?{" "}
          ...
          <div className="availability-heading">
            <div>
              ...
              <p className="availability-legend">
                <span className="legend-free">●</span> disponível{" "}
                <span className="legend-busy">●</span> ocupado{" "}
                <span className="legend-past">●</span> encerrado
              </p>
            </div>

            <button className="refresh-button" onClick={loadBooked}>
              ↻ Atualizar
            </button>
          </div>
          ...
          <div className="time-grid">
            {slots.map((slot) => {
              const occupied = unavailable.some(
                (appointment) => appointment.startTime.slice(0, 5) === slot
              );

              const [slotHour, slotMinute] = slot.split(":").map(Number);
              const slotMinutes = slotHour * 60 + slotMinute;
              const past = isToday && slotMinutes <= currentMinutes;
              const unavailableSlot = occupied || isSunday || past;

              return (
                <button
                  key={slot}
                  disabled={unavailableSlot}
                  className={`time-slot ${time === slot ? "selected" : ""} ${occupied ? "occupied" : ""} ${past && !occupied ? "past" : ""}`}
          ...
          <div className="booking-footer">
            <span>
              {time ? `Selecionado: ${time}` : "Selecione um horário disponível"}
            </span>

            <button
              className="btn btn-primary"
              disabled={!time || isSunday}
              onClick={confirm}
            >
      ...
      {confirmation && (
        <div className="confirmation-overlay" role="dialog" aria-modal="true">
          <div className="confirmation-modal glass">
            ...
            <p>
              Obrigado, <strong>{confirmation.name}</strong>. Preparamos seu
              horário na Barber of Friends.
            </p>
            ...
            <div className="confirmation-details">
              ...
              <span>{confirmation.service}</span>
            </div>
