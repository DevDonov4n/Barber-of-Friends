...
export default function ClientPanel() {
  ...
  const [loyaltyOffer, setLoyaltyOffer] = useState<{
    id: number;
    cutsCount: number;
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(async ([p, a, l]) => {
        if (p.status === 401) {
          router.push("/login?next=/painel/cliente");
          return;
        }
        ...
        if (!p.ok) {
          setError(pd.error || "Não foi possível carregar seu perfil.");
          return;
        }
        setUser(pd.user);
        ...
        setFavoriteCut(pd.user.favoriteCut || "");
        setAppointments(a.ok ? ad : []);
        if (l.ok && ld.offer) {
          setLoyaltyOffer(ld.offer);
          try {
            setLoyaltyOpen(
              localStorage.getItem(`loyalty-offer-${ld.offer.id}`) !==
                "dismissed",
            );
          } catch {
            setLoyaltyOpen(true);
      ...
      .catch(() => setError("Não foi possível carregar sua conta."))
  ...
  async function save(e: FormEvent) {
    ...
    try {
      ...
      setFavoriteCut(data.user.favoriteCut || "");
      setCurrentPassword("");
      setNewPassword("");
      setConfirmPassword("");
  ...
  return (
    <main className="site-shell dashboard-shell">
        ...
        <div className="dashboard-head">
          <div>
            ...
            <p className="page-subtitle">
              Gerencie seus dados e acompanhe seus horários.
            </p>
          </div>
            ...
            <form className="profile-form" onSubmit={save}>
              ...
              <label>
                ...
                <input
                  ...
                  placeholder="Ex.: Taper Fade, Buzz Cut..."
                />
              </label>
                  ...
                  return (
                    <div className="appointment" key={a.id}>
                      ...
                      <time>
                        ...
                        às {formatTime(a.startTime)}
                      </time>
                      {a.status === "CONFIRMED" && (
                        <button
                          className="btn btn-secondary"
                          disabled={cancelId === a.id}
                          onClick={() => setCancelOpen(a.id)}
                        >
                          {cancelId === a.id ? "Cancelando..." : "Cancelar"}
                        </button>
                      )}
                    </div>
                  );
      ...
      {loyaltyOpen && loyaltyOffer && (
        <div
          className="confirmation-overlay loyalty-overlay"
          role="dialog"
          aria-modal="true"
        >
          <div className="confirmation-modal loyalty-modal glass">
            <div className="loyalty-icon">★</div>
            <span className="eyebrow">BENEFÍCIO DE FIDELIDADE</span>
            <h2>Você ganhou 50% de desconto!</h2>
            <p>
              Você completou <strong>5 cortes</strong>. Seu próximo{" "}
              <strong>Corte premium</strong> pode sair pela metade do preço.
            </p>
            <div className="loyalty-price">
              <span>De R$ 80,00</span>
              <strong>Por R$ 40,00</strong>
            </div>
            <p className="loyalty-note">
              O benefício não é obrigatório. Você pode agendar outro corte
              normalmente. Ele só será consumido quando você usar o desconto e o
              barbeiro concluir esse atendimento.
            </p>
      ...
      {cancelOpen !== null && (
        <div className="confirmation-overlay" role="dialog" aria-modal="true">
          <div className="confirmation-modal glass">
            <div className="confirmation-icon confirmation-icon-sad">☹</div>
            <span className="eyebrow">CANCELAR AGENDAMENTO</span>
            <h2>Tem certeza?</h2>
            <p>Se cancelar, esse horário será liberado para outros clientes.</p>
            <div className="confirmation-actions">
              <button
                className="btn btn-secondary"
              ...
              <button
                className="btn btn-primary"
                onClick={() => cancel(cancelOpen)}
                disabled={cancelId !== null}
              >
                {cancelId !== null ? "Cancelando..." : "Sim, cancelar"}
              </button>
            </div>
          </div>
        </div>
